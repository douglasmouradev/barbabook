...
/**
 * BarbaBook - Agendamentos de Barbeiros e Serviços
 */
...
$pageTitle = 'Agendamentos Barbeiro';
$modality  = 'barbeiro';
...
if ($multitenant) {
    ...
    if ($slug === '') {
        header('Location: ' . (SITE_BASE ?: '') . '/barbeiro/estabelecimentos.php');
        exit;
    }
    $stmtE = $pdo->prepare("SELECT id, nome, slug, tipo FROM estabelecimentos WHERE slug = ? AND tipo = 'barbeiro' AND ativo = 1 LIMIT 1");
    $stmtE->execute([$slug]);
...
try {
    $midBarbeiro = (int) $pdo->query("SELECT id FROM modalidades WHERE slug = 'barbeiro' LIMIT 1")->fetchColumn();
    $sqlServ = "SELECT id, nome, duracao_minutos, preco FROM servicos WHERE modalidade_id = ? AND ativo = 1";
    $paramsServ = [$midBarbeiro];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['criar_agendamento']) && csrf_validate()) {
    ...
    if ($ok) {
        try {
            $modalidade_id = (int) $pdo->query("SELECT id FROM modalidades WHERE slug = 'barbeiro' LIMIT 1")->fetchColumn();
            ...
            if ($estabelecimento) {
                ...
                header('Location: ' . (SITE_BASE ?: '') . '/barbeiro/agendamentos.php?e=' . rawurlencode($estabelecimento['slug']) . '&sucesso=1');
                exit;
            }
            $_POST = [];
        } catch (PDOException $e) {
...
if ($telefone_busca !== '') {
    try {
        $sqlMeus = "
            ...
            WHERE a.modalidade_id = (SELECT id FROM modalidades WHERE slug = 'barbeiro' LIMIT 1)
...
<section class="agendamentos-page">
  <div class="container">
    <h1 class="page-title">Agendamentos – Barbeiro<?= $estabelecimento ? ' – ' . htmlspecialchars($estabelecimento['nome']) : '' ?></h1>
    <p class="page-desc">Serviços de corte, barba e cuidados masculinos. Escolha o serviço, o profissional e o horário.</p>
    ...
    <div class="grid-two">
              ...
              <select id="servico_id" name="servico_id" required>
                ...
                <?php foreach ($servicos as $s): ?>
                  <option value="<?= (int)$s['id'] ?>" <?= (int)($_POST['servico_id'] ?? 0) === (int)$s['id'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($s['nome']) ?> – <?= number_format((float)$s['preco'], 2, ',', '.') ?> (<?= (int)$s['duracao_minutos'] ?> min)
                  </option>
                <?php endforeach; ?>
              </select>
            </div>
      ...
      <div class="card list-card">
        <h2>Meus agendamentos – Barbeiro</h2>
        <p class="empty-msg" style="margin-bottom:0.75rem;">Apenas seus agendamentos de barbeiro. Informe seu telefone para consultar.</p>
    ...
    <div class="servicos-box card">
      <h2>Serviços disponíveis – Barbeiro</h2>
